...
if (Test-Path "onyx" -PathType Container)
{
	if (Test-Path (Join-Path "onyx" "onyx") -PathType Container)
	{
		Set-Location "onyx"
	}
	python -m onyx
...
$ret = Invoke-RestMethod -Uri "https://api.github.com/repos/git-for-windows/git/releases" -Headers @{ 'User-Agent' = 'onyx installer' }
...
Invoke-WebRequest -Uri $download_url -OutFile (Join-Path $env:TEMP "git-scm-installer.exe") -Headers @{ 'User-Agent' = 'onyx installer'; 'Accept' = 'application/octet-stream' }
...
git clone https://github.com/onyx/onyx

Set-Location onyx
python -m pip install -r requirements.txt
python -m onyx
python -m onyx --heroku # Stopgap
