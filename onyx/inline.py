...
photo = io.BytesIO(
    requests.get(
        "https://github.com/onyx/onyx/raw/master/onyx/bot_avatar.png"  # noqa: E501, W505
    ).content
)
...
class BotMessage(AiogramMessage):
    def __init__(self) -> None:
        super().__init__(
            ...
            chat=type("Chat", (object,), {"id": 0, "type": "private"})(),
        )  # type: ignore


class onyxInlineQuery:
...
async def edit(
    text: str,
    reply_markup: List[List[dict]] = None,  # type: ignore
    ...
    if reply_markup is not None:
        ...
        if isinstance(reply_markup[0], dict):
            reply_markup = [[_] for _ in reply_markup]  # type: ignore
    ...
    try:  # type: ignore
        await self.bot.edit_message_text(  # type: ignore
            text,
    ...
    except aiogram.exceptions.TelegramAPIError:  # type: ignore # MessageNotModified
        try:
            await query.answer()
        except aiogram.exceptions.InvalidQueryID:  # type: ignore
            pass  # Just ignore that error, bc we need to just
            # remove preloader from user's button, if message_id
            # was deleted
    ...
    except aiogram.exceptions.TelegramBadRequest:  # MessageIdInvalid
        try:
...
class InlineManager:
    def __init__(self, client, db, allmodules) -> None:
        ...
        self._token = db.get("onyx.inline", "bot_token", False)
    ...
    async def _create_bot(self) -> None:
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            # Set its name to user's name + onyx Userbot
            m = await conv.send_message(f"🤖 onyx Userbot of {self._name}")
            r = await conv.get_response()
        ...
        async with self._client.conversation("@BotFather", exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if re.search(r"@onyx_[0-9a-zA-Z]{6}_bot", button.text, re.I):
                        await asyncio.sleep(2)
                        ...
                        self._db.set("onyx.inline", "bot_token", token)
                        self._token = token
                        ...
                        m = await conv.send_message("onyxQuery")
                        r = await conv.get_response()
    ...
    async def _dp_revoke_token(self, inited=True) -> None:
        ...
        self._db.set("onyx.inline", "bot_token", None)
        self._token = None
        if inited:
    ...
    async def _register_manager(
        ...
        self.bot = Bot(token=self._token)  # type: ignore
        ...
        self._dp.inline_query.register(self._inline_handler, lambda inline_query: True)
        ...
        async def new(*args, **kwargs):
            nonlocal revoke, old
            try:
                return await old(*args, **kwargs)  # type: ignore
            except aiogram.exceptions.TerminatedByOtherGetUpdates:
                await revoke()  # type: ignore
            except aiogram.exceptions.Unauthorized:
                logger.critical("Got Unauthorized")
                await self._stop()

        self.bot.get_updates = new  # type: ignore
        ...
        for row in buttons:
            ...
            for button in row:
                try:
                    ...
                    elif "data" in button:  # type: ignore
                        line += [
                            InlineKeyboardButton(
    ...
    async def _inline_handler(self, inline_query: InlineQuery) -> None:
        ...
        for mod in self._allmodules.modules:
            ...
            instance = onyxInlineQuery(inline_query)
    ...
    async def form(
        ...
        if reply_markup:
            if isinstance(reply_markup, dict):  # type: ignore
                reply_markup = [[reply_markup]]
            if isinstance(reply_markup[0], dict):
        ...
        if not isinstance(reply_markup, list):  # type: ignore # This check is redundant due to previous `if reply_markup` block
    ...
    async def gallery(
        ...
        gallery_uid: str = rand(30)  # type: ignore
