...
def translatable_docstring(cls):
    ...
    if hasattr(cls, "config_complete"):
        ...
        def config_complete(self, *args, **kwargs):
            ...
            return cls.config_complete._old_.__get__(self, cls)(*args, **kwargs)

        config_complete._old_ = cls.config_complete # type: ignore

    if not hasattr(cls, '_tds_extracted'):
...
class Modules:
    ...
    def register_module(self, spec, module_name, origin="<file>"):
        ...
        if hasattr(module, "__version__"):
            ret.__version__ = module.__version__  # type: ignore
        ...
        ret.__origin__ = origin # type: ignore
    ...
    def send_config(self, db, translator, skip_hook=False):
        """Configure modules"""
        for mod in self.modules:
            self.send_config_one(mod, db, translator, skip_hook)

    @staticmethod
    def send_config_one(mod, db, translator=None, skip_hook=False):
        ...
        if translator is not None:
            translator.load_translations(mod.__module__, mod.__class__.__module__.__file__)
            mod.strings = translator.get_module_strings(mod.__module__)
    ...
    async def send_ready(self, client, db, allclients):
        ...
        if self.added_modules:
            await self.added_modules(self) # type: ignore
