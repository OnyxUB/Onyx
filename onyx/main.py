...
def get_phones(arguments):
    ...
    phones = {
        ...
        for phone in map(
            lambda f: f[18:-8],
            filter(
                lambda f: f.startswith("onyx-") and f.endswith(".session"),
...
def main():  # noqa: C901
    ...
    while api_token is None:
        ...
        if web:
            ...
            if platform.system() == "Linux" and not os.path.exists("/etc/os-release"):
                ...
                print(f"Please visit http://{ipaddress}:{port}")
        ...
        if web:
            ...
            for client in clients:
                session = SQLiteSession(
                    os.path.join(
                        arguments.data_root or BASE_DIR,
                        f"onyx-+{'X' * (len(client.phone) - 5)}{client.phone[-4:]}",  # skipcq: FLK-E501
                    )
                )
    ...
    for phone_id, phone in phones.items():
        session = os.path.join(
            arguments.data_root or BASE_DIR,
            f"onyx{(('-' + phone_id) if phone_id else '')}",
        )
        ...
        try:
            ...
            client.start()
            client.phone = phone  # type: ignore
            ...
            print(
                ...
                f"'onyx-{phone if phone else ''}.session'"
            )
            continue
...
async def amain(first, client, allclients, web, arguments):
    ...
    if setup:
        db = frontend.get_db(noop=True)
        await db.init()  # type: ignore

        pdb = dict(db)

        modules = loader.Modules(arguments.use_inline)
        translator = Translator(
            target_lang=str(db.get(__name__, "language", "en")),
            fallback_lang="en",
            data_root=arguments.data_root,  # type: ignore
        )
        modules.register_all()
        modules.send_config(db, translator)
        await modules.send_ready(
            client, db, allclients
    ...
    for handler in handlers:
        handler.setLevel(str(db.get(__name__, "loglevel", logging.WARNING)))
    ...
    if not web_only:
        ...
        modules.check_security = dispatcher.check_security  # type: ignore
    ...
    if first:
        try:
            ...
            logging.info(f"=== PLATFORM: {utils.get_platform_name()} ===")
        except Exception:
            logging.exception(
                "Badge error"
