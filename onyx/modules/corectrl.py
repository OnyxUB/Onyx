...
class CoreMod(loader.Module):
    ...
    strings = {
        ...
        "onyx": (
            "🕶 <b>Congrats! You are onyx!</b>\n\n"
            "<b>onyx version: {}.{}.{}</b>\n"
            "<b>Branch: master</b>"
        ),
        "onyx_beta": (
            "🕶 <b>Congrats! You are onyx!</b>\n\n"
            "<b>onyx version: {}.{}.{}beta</b>\n"
            "<b>Branch: beta</b>\n\n"
    ...
    async def ftgvercmd(self, message: Message) -> None:
        ...
        if "beta" in branch:
            await utils.answer(message, self.strings("onyx_beta").format(*ver))
        elif "alpha" in branch:
            await utils.answer(message, self.strings("onyx_alpha").format(*ver))
        else:
            await utils.answer(message, self.strings("onyx").format(*ver))
