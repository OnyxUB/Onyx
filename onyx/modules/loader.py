...
class LoaderMod(loader.Module):
    ...
    strings = {
        ...
        "inline_init_failed": (
            "🚫 <b>This module requires onyx inline feature and initialization of InlineManager failed</b>\n"
            "<i>Please, remove one of your old bots from @BotFather and restart userbot to load this module</i>"
        ),
        "version_incompatible": "🚫 <b>This module requires onyx {}+\nPlease, update with </b><code>.update</code>",
    ...
    def __init__(self):
        ...
        self.config = loader.ModuleConfig(
            "MODULES_REPO",
            "https://raw.githubusercontent.com/onyx/FTG-Modules/main/",
    ...
    async def load_module(
        ...
        if re.search(r"# ?scope: ?onyx_min", doc):
            ver = re.search(
                r"# ?scope: ?onyx_min ([0-9]+\.[0-9]+\.[0-9]+)", doc
            ).group(1)
    ...
    async def unloadmodcmd(self, message: Message) -> None:
        ...
        for mod in worked:
            if not mod.startswith("onyx.modules.") or not mod:
                raise Exception("Assertion error")

            without_prefix += [unescape_percent(mod[len("onyx.modules.") :])]
    ...
    async def clearmodulescmd(self, message: Message) -> None:
        ...
        self._db.set("onyx.modules.loader", "loaded_modules", [])
        self._db.set("onyx.modules.loader", "unloaded_modules", [])
