...
def get_args_split_by(message, sep):
    ...
    mess = raw.split(sep) # type: ignore
...
def get_git_info():
    ...
    return [
        ver,
        f"https://github.com/onyx/onyx/commit/{ver}" if ver else "",
    ]
...
async def answer(message, response, **kwargs):
    ...
    if isinstance(response, str) and not kwargs.pop("asfile", False):
        txt, ent = (parse_mode.parse(response) if hasattr(parse_mode, "parse") else (response, None)) # type: ignore
...
async def get_target(message, arg_no=0):
    ...
    if len(get_args(message)) > arg_no: # type: ignore
        user = get_args(message)[arg_no] # type: ignore
